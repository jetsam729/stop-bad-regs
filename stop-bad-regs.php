...
require_once(__DIR__.'/geoip2lite/geoip2lite.php');	//tempory
...
function ss_styles() {
	...
	if(file_exists(SS_PLUGIN_FILE.'css/custom.css' )){
		wp_enqueue_style( 'custom', plugin_dir_url( __FILE__ ) . 'css/custom.css' );
	}

	if(file_exists(SS_PLUGIN_FILE.'css/iconset-sfs.css' )){
		wp_enqueue_style( 'icoset', plugin_dir_url( __FILE__ ) . 'css/iconset-sfs.css' );
	}

	if(!empty($_GET['page']) && $_GET['page']=='ss_options'){
		wp_enqueue_style( 'flagset2', plugin_dir_url( __FILE__ ) . 'css/flagset2.css' );
	}
